...
def main() -> None:
    df = pd.read_csv("RUCSU election - Sheet1.csv")
    ...
    sections = [
        {
            ...
            "pred_col": "Socchar (VP)",
            "actual_col": "Actual Percentage (VP)",
        },
        {
            "label": "GS",
            "candidate_col": "Candidate (GS)",
            "pred_col": "Socchar (GS)",
            "actual_col": "Actual Percentage (GS)",
        },
        {
            "label": "AGS",
            "candidate_col": "Candidate (AGS)",
            "pred_col": "Socchar (AGS)",
            "actual_col": "Actual Percentage (AGS)",
        },
    ]
    ...
    for section in sections:
        cols = [section["candidate_col"], section["pred_col"], section["actual_col"]]
        section_df = df[cols].rename(
            columns={
                section["candidate_col"]: "Candidate",
                section["pred_col"]: "Predicted",
                section["actual_col"]: "Actual",
            }
        )
        section_df = section_df.dropna(subset=["Predicted", "Actual"])
        if section_df.empty:
            print(f"{section['label']}: no data after filtering.")
            continue

        metrics = compute_metrics(section_df, pred_col="Predicted", actual_col="Actual")
        out_df = pd.DataFrame([{"Office": section["label"], **metrics}])
        print(f"{section['label']} candidates:", ", ".join(section_df["Candidate"]))
        print(out_df)
        all_rows.append(section_df)

        no_others_df = section_df[
            ~section_df["Candidate"].str.contains("Others", case=False, na=False)
        ].copy()
        mov_error = (
    ...
    if all_rows:
        ...
        total_metrics = compute_metrics(all_df, pred_col="Predicted", actual_col="Actual")
        total_df = pd.DataFrame([{"Office": "Total (All)", **total_metrics}])
        print("Total candidates:", ", ".join(all_df["Candidate"]))
        print(total_df)
