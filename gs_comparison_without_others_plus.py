...
def main() -> None:
    ...
    survey_cols = ["Dessent", "Narrative"]
    common_df = df[df[survey_cols].notna().all(axis=1)][
        ["GS candidate", "Actual"] + survey_cols
    ].copy()

    common_df = common_df[common_df["GS candidate"] != "Others+"].copy()

    if common_df.empty:
        raise SystemExit(
            "No common candidates between Dessent and Narrative (excluding Others+)."
        )
